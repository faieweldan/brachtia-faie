...
function PaymentsPage() {
  const { payments, residents } = useOps();
  ...
  const totals = useMemo(() => {
    ...
    return { billed, collected, outstanding: billed - collected, overdue: overdue.length };
  }, [rows]);

  function createInvoice() {
    if (!draft.residentId || !draft.amount || !draft.dueDate) {
      toast.error("Resident, amount and due date are required");
      return;
    }
    addPayment({
    ...
    setOpen(false);
    setDraft({ residentId: "", kind: "rent", label: "", periodFrom: "", periodTo: "", amount: "", dueDate: "" });
  }
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-3 sm:grid-cols-4">
        ...
        <Stat label="Overdue" value={String(totals.overdue)} tone="text-rose-700" />
      </div>

      <Panel
        title="Invoices & payments"
        description="Everything due, paid or overdue across residents."
        action={
          <Button size="sm" onClick={() => setOpen(true)} disabled={residents.length === 0}>
      ...
      >
        {rows.length === 0 ? (
          <EmptyState
            ...
            hint={
              residents.length === 0
                ? "Add a resident first, then raise the booking fee or rent invoice here."
            ...
            <table className="w-full min-w-[860px] text-sm">
              ...
              <tbody className="divide-y divide-border">
                {rows.map((p) => {
                  ...
                  return (
                    <tr key={p.id}>
                      <td className="px-3 py-2 font-medium">{resident?.fullName || "—"}</td>
        ...
        <DialogContent>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <Select
              ...
              onChange={(v) => setDraft((d) => ({ ...d, residentId: v }))}
              options={residents.map((r) => ({ value: r.id, label: r.fullName || "Untitled resident" }))}
            />
      ...
      <Dialog open={!!recording} onOpenChange={(o) => !o && setRecording(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {recording ? `${recording.label} · ${money(recording.amount)}` : ""}
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3 sm:grid-cols-2">
